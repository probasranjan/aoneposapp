...
import com.aoneposapp.utils.JSONLoginFunction;
...
//			Toast.makeText(EmployeeActivity.this, "Server Error", 1000).show();
	...
	void loginFunction() {
		...
		if (Parameters.OriginalUrl.equals("")) {
			...
			if (Parameters.isNetworkAvailable(LoginHomeActivity.this)) {
						...
						+Parameters.randomValue()+"&systemtime="+Parameters.currentTime().replace(" ", "%20");
	...
	void loginWithoutNetConnection(){
		...
		if (uName != null || pWord != null) {
			...
			if (cursorlogin23.getCount() > 0) {
				...
				if (cursoremp23.getCount() > 0) {
					...
					Cursor cursorlogin0123 = dbforloginlogoutRead.rawQuery(
...
LoginReturnJson forlogin = new LoginReturnJson() {
		...
		public void logintoserverinterface(JSONObject jsontext) {
			// TODO Auto-generated method stub
			if(jsontext!=null&&!jsontext.toString().equals("{response: }")){
				Log.v("singam", "singam");
			LoginformServer(jsontext.toString());
			
			}else{
				Log.v("singam", "singam2");
				loginWithoutNetConnection();
	...
	void LoginformServer(String response){
		...
		try {
			...
			// TODO Auto-generated catch block
			System.out.println("error is:harinath" + servertime);
			e.printStackTrace();
		}
		...
		if (mCursors34 != null) {
			...
			if (mCursors34.getCount() < 1) {
				...
				if (mCursors34.moveToFirst()) {
					...
					String url = mCursors34.getString(mCursors34
							.getColumnIndex(DatabaseForDemo.MISCEL_PAGEURL));
					if (url.length() > 3) {
				...
				if (miscurq123.moveToFirst()) {
					do {
						if (miscurq123
							...
							Parameters.OriginalUrl = miscurq123
									.getString(miscurq123
											.getColumnIndex(DatabaseForDemo.MISCEL_PAGEURL));
						}
