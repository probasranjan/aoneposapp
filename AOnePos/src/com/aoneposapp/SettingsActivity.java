...
PrinterEditAdapter.OnWidgetItemClicked {
	...
	long count = 0;
	GetJSONListener l;
	String selectpaymentname = "", selectpaymentval = "", file_Path = null;
	String forurlSave="";
		...
		try{
			...
			database.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					card.setTextColor(Color.WHITE);
					LayoutInflater inflater = getLayoutInflater();
					final View layout = inflater.inflate(R.layout.backup_restore, (ViewGroup) v.findViewById(R.id.inflatingLayout));

					Button restorebutton = (Button) layout .findViewById(R.id.restorebutton);
					Button backupbutton = (Button) layout .findViewById(R.id.backupbutton);
					Button exportbutton = (Button) layout .findViewById(R.id.exportbutton);
					Button importbutton = (Button) layout .findViewById(R.id.importbutton);
						...
						public void onClick(View v) {
							...
							try{
								...
								try {
									...
									source.close();
									destination.close();
									Toast.makeText(SettingsActivity.this, "DB Exported!", Toast.LENGTH_LONG).show();
									showAlertDialogforpermission(
											SettingsActivity.this,
											"File Saved At",
								...
								if (secondaryy.length() > 0) {
									if (mid.length() > 0) {
										...
										if(update_bool){
											...
											contentValues.put(DatabaseForDemo.MERCHANT_NAME, primaryy);
											contentValues.put(DatabaseForDemo.MERCHANT_ADDRESS, secondaryy);
											contentValues.put(DatabaseForDemo.MERCHANT_ADDRESS2, secondaryy2);
											contentValues.put(DatabaseForDemo.MERCHANT_PHONE, secondaryy3);
											contentValues.put(DatabaseForDemo.MERCHANT_ZIP, mid);

											dbforloginlogoutWrite1.insert(DatabaseForDemo.MERCHANT_TABLE, null, contentValues);
											contentValues.clear();	
										}
										Toast.makeText(getApplicationContext(),
												"Saved", 1000).show();
			...
			serverurl.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					database.setBackgroundResource(R.drawable.toprightmenu);
					company.setBackgroundResource(R.drawable.toprightmenu);
					serverurl.setBackgroundResource(R.drawable.highlightedtopmenuitem);
					card.setBackgroundResource(R.drawable.toprightmenu);
					tslookup.setTextColor(Color.WHITE);
					printermainbtn.setTextColor(Color.WHITE);
					tax.setTextColor(Color.WHITE);
					...
					card.setTextColor(Color.WHITE);
					LayoutInflater inflater = getLayoutInflater();
					final View layout = inflater.inflate(R.layout.server_url, (ViewGroup) v.findViewById(R.id.inflatingLayout));

					final Spinner demoselection = (Spinner)layout.findViewById(R.id.demodatatypespinner);
					final EditText mUrl = (EditText) layout .findViewById(R.id.serverurl);
					...
					if (miscursn6.getCount() > 0) {
						if (miscursn6 != null) {
							if (miscursn6.moveToFirst()) {
								do {
									if (miscursn6.isNull(miscursn6 .getColumnIndex(DatabaseForDemo.MISCEL_PAGEURL))) {
										mUrl.setText("");
									} else {
										String urlval = miscursn6.getString(miscursn6 .getColumnIndex(DatabaseForDemo.MISCEL_PAGEURL));
										mUrl.setText(urlval);
								...
								if (url.length() > 3) {
									if (Patterns.WEB_URL.matcher(url).matches()) {
										if (URLUtil.isHttpUrl(url)) {
											...
											Toast.makeText(getApplicationContext(), "URL saved successfully!", Toast.LENGTH_LONG).show();
											Parameters.methodForLogout(SettingsActivity.this);
											finish();
//											showAlertDialogforpermission1(
//													SettingsActivity.this,
...
//													false);
	...
	@SuppressWarnings("deprecation")
	public void showAlertDialogforpermission1(Context context, String title,String message, Boolean status) {
		try{
			AlertDialog alertDialog = new AlertDialog.Builder(context).create();
...
//							}
						...
						{
							Toast.makeText(SettingsActivity.this, "Please Save URL first.", 1000).show();
						}
					}catch(Exception e){
	...
	@SuppressWarnings("deprecation")
	public void showAlertDialogforrestore(Context context, String title, String message, Boolean status) 
	{
		try{
			AlertDialog alertDialog = new AlertDialog.Builder(context).create();
						...
						if (checkRate(tax_value.getText().toString())) {
							...
							if(Parameters.OriginalUrl.equals("")){
								...
								if(Parameters.isNetworkAvailable(SettingsActivity.this)){
									new Thread(new Runnable() {
										...
										public void run() {
											...
											try{
												...
												Cursor cursorz013 = dbforloginlogoutRead1.rawQuery(select, null);
												...
												if (cursorz013.getCount() > 0) {
													...
													dbforloginlogoutRead1.insert( DatabaseForDemo.MISCELLANEOUS_TABLE, null, contentValues1);
												}
											}catch(SQLException e){
												e.getLocalizedMessage();
											}
									...
									ContentValues contentValues1 = new ContentValues();
									contentValues1.put( DatabaseForDemo.QUERY_TYPE, "edit");
									contentValues1.put( DatabaseForDemo.PENDING_USER_ID, Parameters.userid);
									contentValues1.put( DatabaseForDemo.PAGE_URL, "execute.php");
									...
									dbforloginlogoutRead1.insert( DatabaseForDemo.PENDING_QUERIES_TABLE, null, contentValues1);
									dataval = "";
								}
							}
						} else {
							Toast.makeText(SettingsActivity.this, "Enter float or integer value", 1000).show();
						}
					} else {
						Toast.makeText(SettingsActivity.this, "Enter Data", 1000)
						.show();
		...
		try{
			...
			if (cursory201 != null) {
				if (cursory201.moveToFirst()) {
					do {
						String catname = cursory201 .getString(cursory201 .getColumnIndex(DatabaseForDemo.INVENTORY_ITEM_NAME));
						// System.out.println(catname);
						String catid = cursory201.getString(cursory201 .getColumnIndex(DatabaseForDemo.INVENTORY_ITEM_NO));
						// System.out.println(catid);
						String itemcheck = cursory201.getString(cursory201 .getColumnIndex(DatabaseForDemo.CHECKED_VALUE));
		...
		try{
			...
			ok.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					dbforloginlogoutWrite1.delete(DatabaseForDemo.PRINTER_TABLE, where, new String[] { id });

					Toast.makeText(SettingsActivity.this, "Store deleted successfully", Toast.LENGTH_SHORT) .show();
					id_data.clear();
					desc_data.clear();
					total_id_data.clear();
					total_desc_data.clear();
	...
	void sendDatatoServer(String response){
		...
		if(cursorz07!=null){
			if (cursorz07.getCount() > 0) {
				...
				ContentValues contentValues2 = new ContentValues();
				contentValues2 .put(DatabaseForDemo.MISCEL_STORE, "store1");
				contentValues2 .put(DatabaseForDemo.MISCEL_PAGEURL, "");
				contentValues2 .put(DatabaseForDemo.MISCEL_UPDATE_LOCAL, Parameters .currentTime());
				contentValues2 .put(DatabaseForDemo.MISCEL_SERVER_UPDATE_LOCAL, Parameters .currentTime());
				dbforloginlogoutWrite1.insert( DatabaseForDemo.MISCELLANEOUS_TABLE, null, contentValues2);
	...
	public class ExportDatabaseCSVTask extends AsyncTask<String, Void, Boolean>
	{
		...
		{
			File dbFile=getDatabasePath("AonePosDemoDB");
			DatabaseForDemo dbhelper = new DatabaseForDemo(SettingsActivity.this) ;
			...
			try
			{
				...
				SQLiteDatabase db = dbhelper.getWritableDatabase();
				Cursor curCSV = null;
				for(int j=0;j<Parameters.table_names.length;j++)
				{
					curCSV=db.rawQuery("select * from "+Parameters.table_names[j],null);
				
					int count = curCSV.getColumnCount();
					String[] arrHeading = new String[count+2];
					arrHeading[0] = "column_name";
					arrHeading[1] = ""+(count-1);
					for(int i=0;i<count-1;i++)
					{
						arrHeading[i+2] = curCSV.getColumnName(i+1);
					}
//					csvWrite.writeNext(curCSV.getColumnNames());
					csvWrite.writeNext(arrHeading);
					String arrStr[] = new String[count+2];
					while(curCSV.moveToNext())
					{
						arrStr[0] = Parameters.table_names[j];
						arrStr[1] = ""+(count-1);
						for(int i=0;i<count-1;i++)
						{
							arrStr[i+2] = curCSV.getString(i+1);
						}
						csvWrite.writeNext(arrStr);
					}
//					csvWrite.writeNext(new String[2]);
				}
		...
		protected void onPostExecute(final Boolean success)
		{
			...
			if (success)
			{
				Toast.makeText(SettingsActivity.this, "Export succeed", Toast.LENGTH_SHORT).show();
				new Upload_CSV().execute();
			}
			else
			{
	...
	class Upload_CSV extends AsyncTask<Void, Void, String>
	{
		String strUpdate, strPostTo,strMessage;
		...
		{
			super.onPreExecute();
			progressDialog = new ProgressDialog(new ContextThemeWrapper(SettingsActivity.this,R.style.AppBaseTheme));
		...
		protected String doInBackground(Void... params)
		{
			...
			try
			{
				...
				HttpPost post = new HttpPost(Parameters.URL_UploadCSVForSynchronize);
				MultipartEntity reqEntity = new MultipartEntity();
				Log.d("sss","file_Path: "+file_Path);
				if(file_Path!=null)
					reqEntity.addPart("myfile", new FileBody(new File(file_Path))); 
//				reqEntity.addPart(TAG_instructor_id, new StringBody(MainActivity_BcoDemy.str_User_Id));
				...
				if (resEntity != null) 
				{
					Log.d("sss","response_str: "+response_str);
					jsonObject = new JSONObject(response_str);
					Log.d("sss","JSON ADD NOTE : "+jsonObject);
				}
			}
			catch (Exception ex)
			{
